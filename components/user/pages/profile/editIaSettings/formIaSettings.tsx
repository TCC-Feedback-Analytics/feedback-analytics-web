...
export default function FormIaSettings() {
  ...
  const visibleModels = models.filter((model) =>
    model.id === effectiveModel || !onlyFreeModels || isFreeIaModel(model.id),
  );
  const modelOptions: SelectOption<string>[] = [
    ...(activeModel && !models.some((model) => model.id === activeModel)
      ? [{ value: activeModel, label: `${activeModel} — modelo atual indisponível`, disabled: true }]
      : []),
    ...(effectiveModel && effectiveModel !== activeModel && !selectedModel
      ? [{ value: effectiveModel, label: `${effectiveModel} — indisponível`, disabled: true }]
      : []),
    ...visibleModels.map((model) => ({
      value: model.id,
      label: `${model.isAutomatic ? 'Roteamento automático' : model.name} (${model.id})${
        isFreeIaModel(model.id)
          ? ' — Gratuito'
          : onlyFreeModels
            ? ' — seleção atual (fora do filtro de gratuitos)'
            : ''
      }`,
      disabled: onlyFreeModels && !isFreeIaModel(model.id),
    })),
  ];
  ...
  return (
    <div data-tour="ia-settings-form" className="space-y-6">
      ...
      <form onSubmit={handleSave} className="space-y-6" aria-busy={isBusy}>
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onChange={(model) => setModelOption(model)}
              ...
              emptyMessage={onlyFreeModels ? 'Nenhum modelo gratuito corresponde à busca.' : 'Nenhum modelo encontrado para esta busca.'}
            ...
            <div id="modelStatus" role="status" className="space-y-2 text-xs text-(--text-secondary)">
              ...
              {catalog && onlyFreeModels && models.length > 0 && !models.some((model) => isFreeIaModel(model.id)) && <p>Nenhum modelo gratuito encontrado no catálogo atual.</p>}
